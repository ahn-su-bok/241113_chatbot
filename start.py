...
def save_to_vector_store(documents: List[Document], api_key: str) -> None:
...
## 사용자 질문에 대한 RAG 처리
@st.cache_data
def process_question(user_question, api_key):
...
def get_rag_chain(api_key: str) -> Runnable:
    template = """
...
def main():
    st.set_page_config("김포도시관리공사 지침 FAQ 챗봇", layout="wide")
    left_column, right_column = st.columns([1, 1])
    with left_column:
        st.header("김포도시관리공사 지침 FAQ 챗봇")
        ...
        if pdf_doc and button:
            with st.spinner("PDF문서 저장중"):
                ...
                save_to_vector_store(smaller_documents, api_key)
            # (3단계) PDF를 이미지로 변환해서 세션 상태로 임시 저장
            with st.spinner("PDF 페이지를 이미지로 변환중"):
                images = convert_pdf_to_images(pdf_path)
                st.session_state.images = images

        user_question = st.text_input("PDF 문서에 대해서 질문해 주세요", placeholder="초과근무 한도는?")
        
        if user_question:
            response, context = process_question(user_question, api_key)
            st.write(response)
            st.write(context)
            i = 0
